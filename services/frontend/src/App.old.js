...
function App() {
  ...
  const [apiStatus, setApiStatus] = useState({ status: 'loading', services: {} });
  ...
  const availableRules = {
    ...
    completeness: {
      name: '✅ Completeness',
      description: 'Missing values check',
      checks: ['missing_count = 0', 'missing_percent < 10%']
    },
    uniqueness: {
      name: '🔐 Uniqueness',
      description: 'Duplicate detection',
      checks: ['duplicate_count = 0']
    },
    validity: {
      name: '📧 Validity',
      description: 'Format & bounds check',
      checks: ['email format', 'age 13-120']
    },
    freshness: {
      name: '⏰ Freshness',
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setUploadError(error.detail || 'Upload failed. Please try again.');
      }
    } catch (err) {
